...
def print_table(player_dict, country_count):
    print("Players by country:\n----------------------")
    for country, players in sorted(country_count.items()):
        average_elo = 0
        for player in players:
            average_elo += float(player_dict[player][ELO])
        average_elo /= len(players)
        print("{} ({}) ({:.1f}):".format(country, len(players), average_elo))
        for player in players:
...
def main():
    ...
    if file_obj:
        ...
        country_count = count_attribute(player_dict, CON)
        print_table(player_dict, country_count)
